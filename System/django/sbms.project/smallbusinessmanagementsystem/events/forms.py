...
class CreateEventForm(forms.Form):
    ...
    automatic = forms.BooleanField(label=_('Automatic'), widget=forms.CheckboxInput(attrs={'id': 'automatic'}),
                                   required=False)
    date = forms.DateField(label=_('Date'), widget=forms.DateInput(attrs={'type': 'date'}), localize=True)
    time = forms.TimeField(label=_('Time'), widget=forms.TimeInput(attrs={'type': 'time', 'id': 'time'}), localize=True,
                           required=False)
    attendants = forms.IntegerField(label=_('Attendants'), required=False,
                                    widget=forms.NumberInput(attrs={'id': 'attendants'}))
    ...
    validity = forms.ChoiceField(label=_('Validity'), choices=Event.VALIDITIES, required=False,
                                 widget=forms.Select(attrs={'id': 'validity'}))
    ...
    def clean(self):
        ...
        if automatic and charge is None:
            raise ValidationError({'charge': _('Automated events must have a charge.')})


class CreateNewsForm(forms.Form):
    event = forms.ModelChoiceField(label=_('Event'), queryset=Event.objects.filter(date__gt=timezone.now()),
